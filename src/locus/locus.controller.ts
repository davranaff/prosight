...
export class LocusController {
  ...
  @ApiResponse({ status: 200, description: 'Locus data retrieved successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden - insufficient permissions' })
  async findAll(@Query(new ValidationPipe({ transform: true })) query: GetLocusQueryDto, @Request() req) {
    const userRole = req.user.role;

    // Check if non-admin user is trying to use sideloading
    if ((userRole === UserRole.NORMAL || userRole === UserRole.LIMITED) && query.sideload && query.sideload.length > 0) {
